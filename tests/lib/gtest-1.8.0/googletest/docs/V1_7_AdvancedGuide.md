...
Some tips on using `SCOPED_TRACE`:
  ...
  1. When calling sub-routines inside a loop, make the loop iter_ part of the message in `SCOPED_TRACE` such that you can know which iteration the failure is from.
...
| `ValuesIn(container)` and `ValuesIn(begin, end)` | Yields values from a C-style array, an STL-style container, or an iter_ range `[begin, end)`. `container`, `begin`, and `end` can be expressions whose values are determined at run time.  |
